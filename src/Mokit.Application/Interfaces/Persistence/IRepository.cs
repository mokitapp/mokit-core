...
public interface IRepository<T> where T : class
{
    ...
    /// </summary>
    Task<T?> GetByIdWithDeletedAsync(Guid id, CancellationToken cancellationToken = default);

    /// <summary>
    /// Finds records matching condition including soft-deleted ones
    /// </summary>
    Task<IEnumerable<T>> FindWithDeletedAsync(Expression<Func<T, bool>> predicate, CancellationToken cancellationToken = default);

    /// <summary>
    /// Returns queryable ignoring soft delete filter
    /// </summary>
    IQueryable<T> GetQueryableWithDeleted();
}
